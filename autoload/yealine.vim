...
function! yealine#DrawLine(bufno, active)
    if index(yealine#ConfGet("yealine_handle_special", []), getbufvar(a:bufno, "&buftype")) != -1
        let left_boxes = yealine#ConfGet("yealine_special_left_boxes")
        let right_boxes = yealine#ConfGet("yealine_special_right_boxes")
    else
        let left_boxes = yealine#ConfGet("yealine_left_boxes")
        let right_boxes = yealine#ConfGet("yealine_right_boxes")
    endif
...
function! s:register_color(color_spec)
    ...
    if !hlexists(color_name)
        let [tfg, tbg] = [a:color_spec[0], a:color_spec[1]]
        let [gfg, gbg] = [s:xterm_colors[a:color_spec[2]], s:xterm_colors[a:color_spec[3]]]
        execute printf("highlight! %s ctermfg=%s ctermbg=%s guifg=%s guibg=%s", color_name, tfg, tbg, gfg, gbg)
    endif
